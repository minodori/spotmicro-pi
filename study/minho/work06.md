# SpotMicro Week 06 — 서보 조립 시 초기 각도 설정

> 작성일: 2026-07-26

---

## 1. 핵심 원칙: 조립 시 90° 세팅

서보를 조립할 때는 **90°(전기적 중립)로 설정한 뒤 혼(Horn)을 결합**하는 것이 기본 원칙이다.
180° 범위 서보에서 90°는 정중앙이므로 양방향으로 ±90°의 최대 운동 범위를 확보할 수 있다.

단, 이 원칙이 그대로 적용되는 것은 **어깨와 엉덩이뿐**이다. 이 둘은 기준 자세에서 양방향으로
움직이므로 중앙(90)에 두는 것이 맞다. **무릎은 한 방향으로만 움직이므로 기준점을 가동 범위 끝에
두어야 한다** (이유는 §3).

### 1.1 권장 절차 — 무릎은 "다리 편 상태"로 결합

혼 결합 시 목표는 **오프셋이 아래 값 근처에 오는 것**이다. 오프셋만 맞으면 되므로,
결합 전 서보 각도는 관절마다 다르게 잡는 편이 실용적이다.

| 관절 | 결합 전 서보 각도 | 이때 잡을 다리 자세 | 결과 오프셋 |
|------|:-----------------:|---------------------|:-----------:|
| 어깨 (shoulder) | **90°** | 다리가 몸통에서 수직 하방 | ≈ 90 |
| 엉덩이 (upper) | **90°** | 대퇴부 수직 하방 | ≈ 90 |
| 무릎 (lower) — 좌 | **170°** | **다리 완전히 편 상태** (대퇴부·하퇴부 일직선) | ≈ 170 |
| 무릎 (lower) — 우 | **10°** | **다리 완전히 편 상태** | ≈ 10 |

배선이 [work09.md](work09.md) 의 매핑대로 되어 있으면 **인덱스(0~11)로 지정**한다.
보드 주소와 채널은 `_channel_map` 이 알아서 변환하므로 신경 쓰지 않아도 된다.

```bash
python JetsonNano/examples/servo_check.py 0 170    # FL-Lower
python JetsonNano/examples/servo_check.py 6 170    # RL-Lower
python JetsonNano/examples/servo_check.py 3 10     # FR-Lower  (거울 대칭이라 반대편 끝)
python JetsonNano/examples/servo_check.py 9 10     # RR-Lower

python JetsonNano/examples/servo_check.py 1 90     # 나머지 8개(어깨/엉덩이)는 90
python JetsonNano/examples/servo_check.py 2 90
# ... 4, 5, 7, 8, 10, 11 동일
```

**결합 전 각도 한눈에 보기**

| index | 관절 | 각도 | index | 관절 | 각도 |
|:-----:|------|:----:|:-----:|------|:----:|
| **0** | FL-Lower | **170** | **6** | RL-Lower | **170** |
| 1 | FL-Upper | 90 | 7 | RL-Upper | 90 |
| 2 | FL-Shoulder | 90 | 8 | RL-Shoulder | 90 |
| **3** | FR-Lower | **10** | **9** | RR-Lower | **10** |
| 4 | FR-Upper | 90 | 10 | RR-Upper | 90 |
| 5 | FR-Shoulder | 90 | 11 | RR-Shoulder | 90 |

> `raw <보드> <채널> <각도>` 는 **배선 전이거나 매핑 자체를 의심할 때만** 쓴다.
> 배선이 끝난 뒤에는 인덱스가 더 간단하고 실수도 적다.
> ```bash
> python JetsonNano/examples/servo_check.py map        # 인덱스 -> 보드/채널 확인
> python JetsonNano/examples/servo_check.py raw 41 0 170
> ```

> **왜 90° 가 아니라 170°/10° 인가.** 무릎을 90° 에서 결합하려면 "무릎을 약 80° 굽힌 자세"를
> 잡아야 하는데, **80° 굽힘은 눈으로 판정하기 어렵다.** 반면 "다리 완전히 편 상태"는 명확하다.
> 두 방법의 결과 오프셋은 같으므로, 판정하기 쉬운 쪽을 쓴다.
>
> 오른쪽을 1 이 아니라 10 으로 잡는 것은 하한 여유를 두기 위함이다. `offset + theta` 로
> 올라가는 방향이라 손해가 없다.

### 1.1.1 무릎은 한 칸(약 14~18°) 더 안쪽에 물린다 — 2026-08 갱신

위 표의 170/10 으로 작업해도 실측 오프셋은 **179 / 13 / 5** 로 나왔다
([work11.md](work11.md) §6.14). 즉 무릎이 사실상 **가동 한계에 붙어** 있었고, 두 가지 문제를 낳았다.

1. **`servo_check.py home` 이 오프셋을 그대로 명령**하므로 무릎을 내부 스토퍼에 계속 밀어붙인다.
   DS3235 는 신호를 끊어도 setpoint 를 유지하므로 스크립트가 끝난 뒤에도 stall 이 이어진다
2. **무릎은 착지 충격을 정면으로 받는 관절**이다. 앞다리에서 수직 하중의 모멘트 팔이
   Upper 는 0mm, 무릎은 61mm 다. 실제로 RL-Lower / RR-Lower 혼이 반복해서 이탈했다

그래서 결합 전 각도를 **스플라인 한 칸만큼 안쪽으로** 잡는다.

| 관절 | 기존 | **권장** | 결합 후 실측 예 |
|------|:----:|:--------:|:---------------:|
| 무릎 — 좌 (idx 0, 6) | 170 | **160** | ≈ 161 |
| 무릎 — 우 (idx 3, 9) | 10 | **25** | **26** (RR-Lower 실측) |

보행 중 무릎이 쓰는 범위는 한계에서 40° 이상 떨어져 있으므로 **이 이동으로 잃는 것이 없다.**
`home` 도 온전히 쓸 수 있게 된다 (`servo_check.py` 의 `SAFE_BAND = (8, 172)` 밖이면 건너뛴다).

**절차 — 서보를 먼저 돌리고 혼을 끼운다**

```bash
python JetsonNano/examples/servo_check.py 9 25    # ① 혼을 뺀 상태에서 서보를 25도로
                                                   # ② 대퇴부와 하퇴부를 일직선으로 잡는다
                                                   # ③ 그 자세에서 혼을 스플라인에 끼우고 조인다
python JetsonNano/examples/servo_check.py cal 9    # ④ 실측 -> 이 값이 오프셋
```

순서를 뒤집으면(혼을 먼저 끼우고 각도를 맞추면) **한 칸을 반대 방향으로 물릴 위험**이 있다.
좌측 무릎을 잘못 물리면 오프셋이 `179 + 18 = 197` 이 되어 명령 불가능한 값이 된다.

> **스플라인 한 칸이 몇 도인지는 몰라도 된다.** 25T 는 14.4°, 20T 는 18° 다.
> 가장 가까운 칸에 물리고 `cal` 로 실측한 값이 답이다. 목표값은 "이 근처" 라는 뜻이다.

**체결 자체가 더 중요하다.** 혼이 두 번 이탈한 근본 원인은 각도가 아니라 물림이었다.

- 혼을 축에 **끝까지** 밀어넣는다. 덜 들어가면 스플라인 이 일부만 물린다
- 하퇴부 브래킷이 간섭하면 **브래킷 쪽 자리를 갈아** 공간을 확보한다.
  **혼의 스플라인 구멍을 갈면 안 된다** — 물림 면적이 줄어 다시 미끄러진다
- 나사에 **나사풀림방지제**를 바른다. 착지 충격이 반복적으로 나사를 풀어낸다
- 무릎 네 개를 한꺼번에 점검한다. 하나가 빠졌다면 다른 것도 헐겁다

### 1.2 "무릎 80° 굽힘" 의 기하

서보 90° 에서 결합하는 방식을 쓸 경우, 굽힘각을 잘못 이해하기 쉬워 그림으로 남긴다.
**80° 는 수직선에서 벗어난 각도**이지, 수직선에 가까운 각도가 아니다.

```
다리 편 상태 (기준, theta=0)        무릎 80도 굽힘 (서보 90도일 때)

  엉덩관절 O                          엉덩관절 O
           |                                   |
           | 대퇴부 (수직)                      | 대퇴부 (수직)
           |                                   |
  무릎관절 O                          무릎관절 O\
           |                                     \  80도
           | 하퇴부 (일직선)                       \    <- 하퇴부가 수직에서 80도
           |                                        \      (거의 수평)
        발끝 O                                    발끝 O
```

- 대퇴부는 수직 (Upper 조립 때 맞춘 상태)
- 하퇴부는 **수직선에서 80° 벌어짐** → 거의 수평
- 대퇴부–하퇴부 **내각은 100°**

`서보 90° 에서의 굽힘각 = |오프셋 − 90|` 이다. CM4 빌드 실측값 기준:

| 관절 | 오프셋 | 서보 90° 에서의 굽힘 |
|------|:------:|:--------------------:|
| FL-Lower | 150 | 60° |
| RL-Lower | 164 | 74° |
| FR-Lower | 1 | 89° |
| RR-Lower | 1 | 89° |

60~89° 로 흩어진다. 스플라인 한 칸이 18° 라 정확히 맞출 수 없기 때문이며, 이 편차는 오프셋이 흡수한다.

---

## 2. "직립 중립"은 관절마다 다르다

서보를 90°에서 조립했다고 해서, 로봇이 직립 자세를 취할 때 모든 서보가 90°에 있는 것은 아니다.  
`JetsonNano/servo_controller.py`의 offset 배열이 이를 보여준다.

```python
# [FL_무릎, FL_엉덩, FL_어깨, FR_무릎, FR_엉덩, FR_어깨,
#  RL_무릎, RL_엉덩, RL_어깨, RR_무릎, RR_엉덩, RR_어깨]
self._servo_offsets = [170, 85, 90, 1, 95, 90, 172, 90, 90, 1, 90, 95]
```

> **주의**: 위 배열은 Jetson 시절 **다른 개체**의 값이다. 오프셋은 혼이 스플라인에 물린 위치에 따라 개체마다 달라지므로 그대로 쓰면 안 된다. 현재 CM4 빌드의 실측값과 측정 절차는 [work11.md §6.6](work11.md) 참고.

| 관절 | offset 범위 | 의미 |
|------|-------------|------|
| **어깨 (roll/abduction)** | ≈ 90 | 다리 수직 하방 = 좌우 대칭 중립 → 90°가 실제 중립 |
| **엉덩이 (hip/upper leg)** | 85 ~ 95 | 대퇴부 직하방 ≈ 90° 근방 |
| **무릎 (knee/lower leg)** | 170 또는 1 | 좌/우 서보가 **거울 대칭으로 반전 장착** |

---

## 3. 무릎(lower) 서보 offset이 극단값인 이유

`angleToServo()` 함수를 보면 좌/우 무릎은 부호가 반대다.

```python
# FL Lower (왼쪽): 빼기
self._val_list[0] = self._servo_offsets[0] - self._thetas[0][2]  # offset=170

# FR Lower (오른쪽): 더하기
self._val_list[3] = self._servo_offsets[3] + self._thetas[1][2]  # offset=1
```

왼쪽과 오른쪽 무릎 서보는 물리적으로 **반대 방향으로 장착**되어 있기 때문에, 같은 "다리 완전히 편 상태(theta=0)"가:
- 왼쪽 무릎: 서보 **170°**
- 오른쪽 무릎: 서보 **1°**

에 해당한다. 즉 서보를 90°로 조립하고 혼을 달면, 그 상태는 "무릎이 약 80° 구부러진 중간 자세"가 된다.

### 왜 무릎은 편 상태로 결합하면 안 되는가

무릎은 기준 자세(다리를 편 상태, theta=0)에서 **한 방향으로만** 굽는다. 보행 중 최대 theta 는 약 130° 다.

무릎을 **편 상태**에서 서보 90° 로 결합하면 오프셋이 90 이 되고, 실제 보행에서 범위를 벗어난다.

```
왼쪽 무릎:  90 - 130 = -40    <- 동작 불가
```

**80° 굽힌 상태**에서 90° 로 결합하면 오프셋이 170 이 되어 정상 동작한다.

```
왼쪽 무릎: 170 - 130 =  40    <- 정상
```

어깨·엉덩이는 기준 자세에서 양쪽으로 움직이므로 중앙(90)에 두는 것이 맞고,
무릎은 한쪽으로만 움직이므로 기준점을 가동 범위 끝에 두어야 한다. 원칙("90° 에서 결합")은
같지만, **무릎은 그 90° 가 곧 기준 자세는 아니다.**

---

## 4. 실제 직립 자세 = IK가 계산한 각도

`servo_controller.py` 하단의 직립 목표 좌표:

```python
legEndpoints = np.array([[100,-100,87.5,1], [100,-100,-87.5,1],
                          [-100,-100,87.5,1], [-100,-100,-87.5,1]])
thetas = kn.initIK(legEndpoints)
```

발끝이 엉덩이 관절 아래 **100mm** 위치일 때, IK가 계산한 theta 값이 "직립 서보 각도"다.  
이 때 무릎은 완전히 펴지지 않고 **약간 굽혀진 상태**다.

> 다리를 완전히 펴면 역운동학(IK)이 특이점(Singularity)에 빠지고, 충격 흡수 능력도 사라지기 때문에 실제 보행 로봇은 무릎을 항상 약간 구부린 상태로 유지한다.

---

## 5. 조립 권장 순서

1. **서보를 결합 각도로 설정** — 어깨·엉덩이는 90°, 무릎은 좌 170° / 우 10° (§1.1 표 참고)
   ```bash
   python JetsonNano/examples/servo_check.py 4 90          # 인덱스 (배선 완료 후 권장)
   python JetsonNano/examples/servo_check.py raw 40 14 90  # 배선 전에만 보드/채널로
   ```
2. 그 상태에서 아래 다리 자세를 잡고 혼 결합:

   | 관절 | 결합 전 서보 | 혼 결합 시 잡을 다리 자세 | 결과 오프셋 |
   |------|:------------:|---------------------------|-------------|
   | 어깨 (shoulder/roll) | 90° | 다리가 몸체에서 수직 하방 | ≈ 90 |
   | 엉덩이 (hip/upper) | 90° | 대퇴부 직하방 | ≈ 90 |
   | 무릎 (knee/lower) 좌 | **170°** | **다리 완전히 편 상태** | ≈ 170 |
   | 무릎 (knee/lower) 우 | **10°** | **다리 완전히 편 상태** | ≈ 10 |

   > 무릎을 서보 90° 에서 결합하려면 "약 80° 굽힌 자세"를 잡아야 하는데 눈으로 판정하기 어렵다.
   > 서보를 미리 170°/10° 에 두고 **다리를 편 상태**로 결합하면 결과는 같으면서 판정이 명확하다 (§1.1).
   > 좌/우 무릎은 거울 대칭으로 반전 장착되므로 값이 반대편 끝이다.

3. 조립 후 실측 조정 — `JetsonNano/examples/servo_check.py cal <인덱스>`
   (방향키로 1도씩 조정하며 기준 자세를 찾는다. 관절별 목표 자세를 화면에 띄워준다)
4. 측정값을 `servo_controller.py`의 `_servo_offsets` 배열에 기록

> **혼은 원하는 각도에 정확히 안 맞는다.** 스플라인 한 칸이 약 18° 이므로 가장 가까운 칸에 끼우고,
> 남은 오차(최대 9°)를 오프셋으로 흡수한다. 실측 오프셋이 80~95 사이에 흩어지는 것이 정상이다.
>
> 다만 **한 다리만 유독 크게 벌어지면 조립 이상을 의심할 것.** 실제로 FR-Upper 가 다른 Upper(80~81)보다
> 14° 튀었고, 원인은 서보 혼 이탈이었다 ([work11.md §6.13](work11.md)).

---

## 5.1 실측 조정 체크리스트 (RPi5, `test_servos_cali.py`)

`servo_controller.py`의 기존 `_servo_offsets` 값을 그대로 입력해서, RPi5로 이식한 뒤에도 실제 자세가 맞는지 서보별로 검증한다.

| 서보 번호 | 관절 | 입력할 각도 | 기대하는 실제 모습 |
|---|---|---|---|
| 0 | FL(왼앞) 무릎 | 170 | 하퇴부가 완전히 펴진 직하방 |
| 1 | FL 엉덩이 | 85 | 대퇴부 직하방 |
| 2 | FL 어깨 | 90 | 다리가 몸체에서 수직 하방 |
| 3 | FR(오른앞) 무릎 | 1 | 하퇴부 완전히 펴진 직하방 (왼쪽과 반대 방향 장착이라 값도 반대) |
| 4 | FR 엉덩이 | 95 | 대퇴부 직하방 |
| 5 | FR 어깨 | 90 | 다리가 몸체에서 수직 하방 |
| 6 | RL(왼뒤) 무릎 | 172 | 하퇴부 완전히 펴진 직하방 |
| 7 | RL 엉덩이 | 90 | 대퇴부 직하방 |
| 8 | RL 어깨 | 90 | 다리가 몸체에서 수직 하방 |
| 9 | RR(오른뒤) 무릎 | 1 | 하퇴부 완전히 펴진 직하방 |
| 10 | RR 엉덩이 | 90 | 대퇴부 직하방 |
| 11 | RR 어깨 | 95 | 다리가 몸체에서 수직 하방 |

순서대로 0번부터 11번까지 입력해서 각 다리가 기대하는 모습대로 움직이는지 확인한다.

- 전부 맞으면: 기존 오프셋이 여전히 유효 → RPi5 이식 성공.
- 특정 서보가 어긋나 있으면: 조립이 바뀌었거나 서보가 재장착된 것이므로 위 3~4번 순서대로 실측해서 `_servo_offsets` 값을 새로 기록한다.

---

## 6. 관련 파일

| 파일 | 역할 |
|------|------|
| `JetsonNano/servo_controller.py` | `_servo_offsets` 배열, `angleToServo()` 매핑 로직 |
| **`JetsonNano/examples/servo_check.py`** | **조립·캘리브레이션 주력 도구** (아래 참고) |
| `JetsonNano/servo_controller_fix.py` | 표준 기준값 (모두 90°/1°/180°) |
| `JetsonNano/examples/test_servos_cali.py` | 서보별 각도 수동 조정 (1도 단위 스위프) |
| `JetsonNano/examples/test_servos_offset.py` | 오프셋 테스트 (기준: 90°) |
| `Kinematics/kinematics.py` | IK 계산 (`legIK`, `initIK`) |

### `servo_check.py` — 조립 시 자주 쓰는 명령

```bash
cd ~/Projects/SpotMicroJetson

# 혼 결합 전, 해당 서보를 90도(전기적 중립)로
python JetsonNano/examples/servo_check.py 4 90
python JetsonNano/examples/servo_check.py raw 40 14 90   # 배선 전이면 보드/채널 직접 지정

python JetsonNano/examples/servo_check.py map      # 인덱스 -> 보드/채널 표
python JetsonNano/examples/servo_check.py home     # 12개를 기준 자세로 한번에
python JetsonNano/examples/servo_check.py cal 4    # 관절 하나 대화형 캘리브레이션
python JetsonNano/examples/servo_check.py scan     # 12채널 순차 스윕 (배선 실측)
```

조립 직후 좌우 대칭을 볼 때는 `home` 자세가 유리하다. 다리가 완전히 펴진 수직 직선이라
어긋남이 바로 드러난다 (직립 자세는 무릎이 굽어 판정이 어렵다).
